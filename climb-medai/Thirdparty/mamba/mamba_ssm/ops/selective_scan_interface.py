...
# climb-medai patch (upstream imports this unconditionally, unlike
# causal_conv1d_cuda right above): MAMBA_SKIP_CUDA_BUILD=TRUE deliberately
# skips building this extension -- see Thirdparty/mamba's own setup.py -- and
# the model this repo actually runs (Mamba2.inference_forward, in
# colonstreamsfsnet/model/temporal_mamba.py) never calls selective_scan_fn or
# mamba_inner_fn, only the legacy Mamba (v1) class and the reference
# ssd_selective_scan() path below do. Guarding this import the same way
# causal_conv1d_cuda already is lets `import mamba_ssm` succeed without the
# compiled extension; the two functions below would still raise a clear
# AttributeError if something actually tried to call them.
try:
